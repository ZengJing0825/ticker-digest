...
面向**美股与加密的自选股和持仓**的事件驱动推送引擎：一套信号，切成自选日报、观点日报、持仓日报。没事就闭嘴，有事说人话。适用范围：个人或小团队的自选与持仓提醒；实时触发只覆盖美股和加密，港股、日韩等市场只走财报与催化剂日历。默认演示完全离线，使用内置的合成数据。

> 非投资建议。样例数据全部为合成数据（价格、日历、持仓、观点账号都是虚构的），工具只报告规则触发的事实。

## 解决什么问题 / 给谁用

**解决什么问题。** 自选和持仓的「日报」大多是行情聚合：每天一份、每个标的一段、有没有事都发。结果是两种失败：滞后（等你看到，量化早就拉过一波）和过密（用户直接静音）。这个仓库把日报做成一个推送引擎：只有规则触发、通过频控和去重、通过输出契约的内容才会出现，其它标的归入一句「+N more quiet」。

**给谁用。** 给自己维护自选/持仓、想要「有事才响」的个人投资者；给要在产品里做提醒类推送、想先把「推什么、不推什么、为什么不推」这套规则跑通的产品和内容同学；给想在自己的数据源上扩展规则的开发者。它不是行情看板，也不接券商下单，所有输出是带来源和日期的 Markdown。

## 核心逻辑

这个仓库是我把在前一家公司做「自选和持仓日报」时想清楚的东西，从零重写成通用版。它不是行情看板，是一个「没事就闭嘴、有事说人话」的推送引擎。

**目的。** 日报的价值不在聚合，在时效和「帮你想下一步」。推送一旦滞后，量化早就拉过一波了；推送一旦太密，用户直接静音。所以第一条原则是无事不推，第二条是注意力预算：每个标的每天最多 2 条，每份最多 3 个标的。

**架构。** 信号 → 切片 → 排序 → 渲染 → 投递 → 审计。信号层只产出候选，每条带阈值检查和指纹；同一批候选可以按标的切成自选日报、按持仓切成持仓日报、按人切成观点日报。这是我最想保留的抽象：一套信号，多种切片，不为每种日报各写一套抓取。

**排序为什么这样。** 有仓位就按权重乘波动排，风险线（浮盈亏穿越、集中度）优先于纯价格；没仓位就按事件优先级。新鲜度靠指纹去重，同一件事 7 天内不重复，只在恶化时重推。
...
**取舍。** 实时触发只做美股和加密，其他市场只走财报和催化剂日历，原因不是技术，是实时行情数据源太贵。三种持仓接入（手填、截图、连券商）里前两种是静态快照，要在合适节点提醒用户「这条用的是 14 天前的仓位」。

**过程数字（我的原始记录）。** 原型阶段实测一小时推了 5 条高度重复的提醒，一天 16 推 3 静默，才补上「同主题只在恶化时重推加最小间隔」；投放数据里 digest 类聚合页转化垫底，说明日报是基本盘，实时单推才是卖点。
...
**扩展数据源**：继承 `digest.sources.base.DataSource`，实现 `daily_bars(ticker, end)`（最旧在前、不晚于 `end`）和 `earnings_calendar()`，可选实现 `notes()`；失败返回空列表而不是抛异常；在 `digest/cli.py` 的 `build_source()` 注册。**扩展规则**：在 `digest/signals.py` 写一个对触发和未触发都返回 `Signal` 的函数，选好 `priority`、定义 `fingerprint` 的身份含义、给 `severity` 一个「越大越糟」的量，从 `evaluate()` 调用；`headline` 里不要带引用（渲染器会加），也不要出现契约禁止的词，否则渲染会拒绝。
